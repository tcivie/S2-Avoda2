...
int String::countStrlen(const char* string) const {
	int counter = 0;
...
char* String::strCpy(char* string1, const char* string2) {
	char* returned = string1; // create init pointer copy
	while (*string2 != '\0') { // run till end of string 2
		*string1++ = *string2++; // copy values
	}
	*string1 = '\0'; // add null
	return returned; // return so values wont be deleted
}
